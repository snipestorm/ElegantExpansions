...
public class ModFlammableRotatedPillarBlock extends RotatedPillarBlock {
    ...
    public @Nullable BlockState getToolModifiedState(BlockState state, UseOnContext context, ToolAction toolAction, boolean simulate) {
        if (context.getItemInHand().getItem() instanceof AxeItem) {
            ...
            if (state.is(ModBlocks.ECHO_LOG.get())) {
                return ModBlocks.STRIPPED_ECHO_LOG.get().defaultBlockState().setValue(AXIS, state.getValue(AXIS));
            }
            if (state.is(ModBlocks.ECHO_WOOD.get())) {
                return ModBlocks.STRIPPED_ECHO_WOOD.get().defaultBlockState().setValue(AXIS, state.getValue(AXIS));
            }
