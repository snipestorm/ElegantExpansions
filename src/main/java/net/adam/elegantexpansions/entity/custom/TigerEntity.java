...
public class TigerEntity extends Animal implements GeoEntity {
    private static final Predicate<LivingEntity> PARENT_TIGER_SELECTOR = (parent) -> {
        return parent instanceof TigerEntity && ((TigerEntity)parent).isBred();
    };
    ...
    public boolean canMate(Animal p_30698_) {
        if (p_30698_ == this) {
            return false;
        } else if (!(p_30698_ instanceof WhiteTigerEntity) && !(p_30698_ instanceof TigerEntity)) {
            return false;
        } else {
    ...
    public AgeableMob getBreedOffspring(ServerLevel level, AgeableMob ageablemob) {
        if (ageablemob instanceof WhiteTigerEntity) {
             ...
             return ModEntityTypes.TIGER.get().create(level);
