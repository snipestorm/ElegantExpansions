...
public class WhiteTigerEntity extends TigerEntity implements GeoEntity {
    ...
    private static final Predicate<LivingEntity> PARENT_W_TIGER_SELECTOR = (parent) -> {
        return parent instanceof WhiteTigerEntity && ((WhiteTigerEntity)parent).isBred();
    };
    private static final TargetingConditions MOMMY_TARGETING = TargetingConditions.forNonCombat().range(16.0D).ignoreLineOfSight().selector(PARENT_W_TIGER_SELECTOR);
    ...
    public static void entitySpawnRestriction(SpawnPlacementRegisterEvent event) {
        event.register(ModEntityTypes.WHITE_TIGER.get(), SpawnPlacements.Type.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,
    ...
    public AgeableMob getBreedOffspring(ServerLevel level, AgeableMob ageableMob) {
        return ModEntityTypes.WHITE_TIGER.get().create(level);
    }
    ...
    protected void followParent() {
        if (this.isBred() && this.isBaby()) {
            LivingEntity livingentity = this.level().getNearestEntity(WhiteTigerEntity.class, MOMMY_TARGETING, this, this.getX(), this.getY(), this.getZ(), this.getBoundingBox().inflate(16.0D));
